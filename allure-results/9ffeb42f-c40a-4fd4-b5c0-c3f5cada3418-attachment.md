...
- Name: 07_WebTables\Task_StaticTable.spec.ts >> Verify country of a specific user-Method-2
- Location: tests\07_WebTables\Task_StaticTable.spec.ts:17:5
...
Call log:
  - waiting for locator('tr[data-testid=\'row-adobe\'] td').filter({ hasText: 'Yoshi Tannamuri' }).locator('td[data-col=\'country\']')
...
```ts
  ...
  2  | 
  3  | // Method-1: Using XPath to get the country of a specific user
  4  | test("Verify country of a specific user", async({page}) => {
  5  | 
  6  | await page.goto("https://app.thetestingacademy.com/playwright/tables/webtable");
  7  | 
  8  | // //tr[@data-testid="row-adobe"]/td[2]
  9  | 
  10 | 
  11 | const country = await page.locator("//td[text()='Yoshi Tannamuri']/following-sibling::td").innerText();
  12 | console.log(country);
  13 | 
  14 | });
  15 | 
  16 | // Method-2: Using XPath to get the country of a specific user
  17 | test("Verify country of a specific user-Method-2", async({page}) => {
  18 | 
  19 | await page.goto("https://app.thetestingacademy.com/playwright/tables/webtable");
  20 | 
  21 | let name = "Yoshi Tannamuri";
  22 | const row = page.locator("tr[data-testid='row-adobe'] td").filter({hasText: name});
> 23 | const country = await row.locator("td[data-col='country']").innerText();
     |                                                             ^ Error: locator.innerText: Test timeout of 30000ms exceeded.
  24 | console.log(country);
  25 | 
  26 | });
```