...
```ts
  ...
  17 |         
  18 | 
  19 |         const chkBox1 = await page.getByRole('checkbox',{name:'/UFT/'});
  20 |         //const automationCheckBox = page.getByRole('checkbox', { name: /UFT/ });
  21 |         //chkBox.check();
  22 | 
  23 |         //await expect(chkBox1).not.toBeChecked();
> 24 |         await expect(chkBox1).not.toBeChecked();
     |                                   ^ Error: expect(locator).not.toBeChecked() failed
  25 |         await page.pause();      
  26 | 
  27 | 
