...
- Name: 19_Data_Driven_Testing\09_YAML_DDT.spec.ts >> DDT YAML >> Login with : invalid email format
- Location: tests\19_Data_Driven_Testing\09_YAML_DDT.spec.ts:21:13
...
```ts
  ...
  2  | import { loadYAMLTestData } from './util/yamlReader';
  3  | import path from 'path';
  4  | 
  5  | // Load YAML test data synchronously
  6  | const yamlFilePath = path.join(__dirname, './test-data/registration-data.yaml');
  7  | const loginData = loadYAMLTestData(yamlFilePath);
  8  | 
  9  | test.describe('DDT YAML', () => {
  10 | 
  11 |     test.beforeEach(async ({ page }) => {
  12 |         await page.goto('https://app.thetestingacademy.com/playwright/multiple_element_filter');
  13 |     });
  14 | 
  15 |     test.afterEach(async ({ }, testInfo) => {
  16 |         console.log(`afterEach: ${testInfo.title} — status: ${testInfo.status}`);
  17 |     });
  18 | 
