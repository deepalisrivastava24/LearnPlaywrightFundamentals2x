...
- Name: 19_Data_Driven_Testing\07_DDT_CSV.spec.ts >> DDT CSV >> Login with : invalid password
- Location: tests\19_Data_Driven_Testing\07_DDT_CSV.spec.ts:10:13
...
```ts
  ...
  3  | import { readCSV } from './util/csvReader';
  4  | 
  5  | test.describe('DDT CSV', () => {
  6  |     const loginData = readCSV(path.join(__dirname, '/test-data/login-data.csv'));
  7  |     
