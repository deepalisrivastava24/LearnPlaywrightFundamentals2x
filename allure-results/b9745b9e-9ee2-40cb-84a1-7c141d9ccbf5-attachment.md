# Instructions

- Following Playwright test failed.
- Explain why, be concise, respect Playwright best practices.
- Provide a snippet of code with the fix, if possible.

# Test info

- Name: 10_Keyboard_Hover_Drag_Drop\02_SpiceJet_Hover.spec.ts >> Verify keyboard hover and drag and drop
- Location: tests\10_Keyboard_Hover_Drag_Drop\02_SpiceJet_Hover.spec.ts:3:5

# Error details

```
Error: expect(page).toHaveTitle(expected) failed

Expected: "SpiceJet"
Received: "SpiceJet - Flight Booking for Domestic and International, Cheap Air Tickets"
Timeout:  5000ms

Call log:
  - Expect "toHaveTitle" with timeout 5000ms
    7 × unexpected value "SpiceJet - Flight Booking for Domestic and International, Cheap Air Tickets"

```

```yaml
- link "sjlady":
  - /url: /
  - img "sjlady"
- link "sjlady":
  - /url: https://corporate.spicejet.com/SpiceMax.aspx
  - img "sjlady"
- link "sjlady":
  - /url: https://corporate.spicejet.com/VisaServices.aspx
  - img "sjlady"
- link "sjlady":
  - /url: https://spiceclub.spicejet.com/axisBank
  - img "sjlady"
- button
- button
- button
- button
- text: Add-ons
- img
- img
- link "SpiceMax":
  - /url: https://corporate.spicejet.com/SpiceMax.aspx
- link "SpiceCafé":
  - /url: https://corporate.spicejet.com/spiceCafe.aspx
- link "You1st":
  - /url: https://corporate.spicejet.com/YouFirst.aspx
- link "Visa Services":
  - /url: https://corporate.spicejet.com/VisaServices.aspx
- link "SpiceAssurance":
  - /url: https://corporate.spicejet.com/SpiceAssurance.aspx
- link "Friends and Family":
  - /url: https://corporate.spicejet.com/FamilyAndFriendOffer.aspx
- link "Senior Citizen Discount":
  - /url: https://corporate.spicejet.com/SeniorCitizen.aspx
- link "Student Discount":
  - /url: https://corporate.spicejet.com/StudentDiscountLanding.aspx
- link "Extra Seat":
  - /url: https://corporate.spicejet.com/ExtraSeat.aspx
- link "SpiceLock":
  - /url: https://corporate.spicejet.com/spicelock.aspx
- link "Indian Armed Forces Personnel":
  - /url: https://corporate.spicejet.com/IAFPersonnel.aspx
- link "SpiceFlex":
  - /url: https://corporate.spicejet.com/SpiceFlexFare.aspx
- link "Govt. Employee":
  - /url: https://corporate.spicejet.com/GovtEmployee.aspx
- link "SpicePlus":
  - /url: https://corporate.spicejet.com/spicePlus.aspx
- link "FlyEarly":
  - /url: https://corporate.spicejet.com/FLYEarlyProductatAirports.aspx
- link "Excess Baggage":
  - /url: https://corporate.spicejet.com/ExcessBaggage.aspx
- link "Deals":
  - /url: https://corporate.spicejet.com/RedHotOffers.aspx
- link "GiftCard":
  - /url: https://spicejet.woohoo.in/home
- text: SpiceClub
- img
- img
- text: About SpiceClub
- link "Our Program":
  - /url: https://spiceclub.spicejet.com/home#program
- link "Benefits":
  - /url: https://spiceclub.spicejet.com/home#benefits
- link "Tiers":
  - /url: https://spiceclub.spicejet.com/home#tiers
- text: SpiceClub Points
- link "Earn Points":
  - /url: https://spiceclub.spicejet.com/howToEarn
- link "Use Points":
  - /url: https://spiceclub.spicejet.com/howToUse
- text: SpiceClub Exclusives
- link "Credit Cards":
  - /url: https://spiceclub.spicejet.com/axisBank
- link "Instant Vouchers":
  - /url: https://spiceclub.spicejet.com/instantVouchers
- link "SpiceScreen":
  - /url: https://spicescreen.com/
- link "Cargo":
  - /url: https://www.spicexpress.com
- link "Charter":
  - /url: https://corporate.spicejet.com/CharterLanding.aspx
- text: Travel Policies
- img
- img
- link "Passenger Support":
  - /url: https://corporate.spicejet.com/PassengerSupport.aspx
- link "Passenger Rights":
  - /url: https://corporate.spicejet.com/PassengerRights.aspx
- link "Tariffs":
  - /url: https://corporate.spicejet.com/Content/pdf/Tariffs.pdf
- link "Baggage Information":
  - /url: https://corporate.spicejet.com/Baggage.aspx
- img
- text: Login
- link "Signup":
  - /url: https://spiceclub.spicejet.com/signup
- img
- text: Flights
- img
- text: check-in
- img
- text: flight status
- img
- text: manage booking Welcome aboard Let's go places!
- link "Book your Charter":
  - /url: https://corporate.spicejet.com/CharterLanding.aspx
- img
- text: one way
- img
- text: round trip From
- textbox: Delhi (DEL)
- img
- text: To
- textbox: Select Destination
- text: Departure Date Fri, 14 Aug 2026
- img
- text: Return Date Select Date
- img
- text: Passengers 1 Adult
- img
- text: Currency INR
- img
- img
- text: Family & Friends
- img
- text: Senior Citizen
- img
- text: Unaccompanied Minor
- img
- text: Students
- img
- text: Armed Forces
- img
- text: Govt. Employee
- link "Special Assistance":
  - /url: https://corporate.spicejet.com/SpecialAssistance.aspx
  - img
  - text: Special Assistance
- text: Search Flight Due to the onset of the summer schedule from 29 March, flight timings may change. Passengers are advised to check their flight status before departure.
- link:
  - /url: ""
- text: Due to ongoing developments in the Middle East region, some flights may be impacted. Please check your
- link "flight status":
  - /url: https://www.spicejet.com/#status
- text: for updates. Passengers carrying arms & ammunition must request clearance at least 72 hours before departure. Email custrelations@spicejet.com for assistance.
- link "Read more":
  - /url: " https://corporate.spicejet.com/Tnc.aspx"
- text: . Use our
- link "Change Assist":
  - /url: https://changes.spicejet.com/index.html#/
- text: portal for easy rescheduling or refunds for your affected flight. Only 1 piece of hand baggage of up to 7 kg is permitted on all SpiceJet flights. Power banks/portable mobile chargers are ONLY allowed in Cabin Baggage. E-cigarettes are prohibited to be carried in Cabin as well as Checked-in Baggage.
- link:
  - /url: ""
- text: With DigiYatra, enjoy quick and easy entry to the airport, and pre-security check with your face as your boarding pass for the next SpiceJet flight at select cities.
- link "Know more.":
  - /url: https://corporate.spicejet.com/Digiyatra.aspx
- text: I Am Here For...
- link "sjlady Change Assist (Choose alternate flight or refund for changed/cancelled flights)":
  - /url: https://changes.spicejet.com/index.html#/
  - img "sjlady"
  - text: Change Assist (Choose alternate flight or refund for changed/cancelled flights)
- link "sjlady SpiceClub":
  - /url: https://spiceclub.spicejet.com/
  - img "sjlady"
  - text: SpiceClub
- link "sjlady SpiceJet Axis Bank Credit Card":
  - /url: https://spiceclub.spicejet.com/axisBank
  - img "sjlady"
  - text: SpiceJet Axis Bank Credit Card
- link "sjlady Visa Services":
  - /url: https://corporate.spicejet.com/VisaServices.aspx
  - img "sjlady"
  - text: Visa Services
- link "sjlady GST Invoice":
  - /url: ""
  - img "sjlady"
  - text: GST Invoice
- link "Spin to Win":
  - /url: ""
  - text: Spin to Win
  - link:
    - /url: ""
- text: Irresistible Deals SpiceMax The best seat in the sky at 30% Off SMAX30 Valid till:September 30, 2026 Enjoy ADDONS at Upto 25% Off ADDON25 Valid till:September 30, 2026 Explore More Recommended Add-Ons
- link "SpiceMax Upgrade your flying experience with more space and a host of convenient services and priority privileges.":
  - /url: https://corporate.spicejet.com/spiceMax.aspx
- link "SpiceCafé Pamper your taste buds with our variety of delicious hot meals and sandwiches.":
  - /url: https://corporate.spicejet.com/spiceCafe.aspx
- link "Excess Baggage Avoid last-minute baggage hassles at the airport by pre-booking your excess baggage now and save up to 10%!":
  - /url: https://corporate.spicejet.com/ExcessBaggage.aspx
- link "You1st Enjoy Priority Check-in, Boarding & Baggage Delivery for just ₹595/₹699 (Domestic/International)!":
  - /url: https://corporate.spicejet.com/YouFirst.aspx
- link "Extra Seat Enjoy added privacy, extra space, and up to 10kg additional baggage allowance with an extra seat or row!":
  - /url: https://corporate.spicejet.com/ExtraSeat.aspx
- link "SpicePlus Enjoy priority services with a meal and beverage at ₹500. Add your preferred seat for an extra cost.":
  - /url: https://corporate.spicejet.com/spicePlus.aspx
- text: Tailor-made For You
- link "Group Bookings Quick responses, effortless tracking, and a seamless booking journey.":
  - /url: https://groups.spicejet.com/
- link "SME High Flyer Exclusive fares, flexible changes & premium perks for small and medium enterprises.":
  - /url: https://sme.spicejet.com/Account/Login?ReturnUrl=%2FHome%2FLandingPage
- link "Corporate Traveller Customised travel solutions for business professionals.":
  - /url: https://corporate.spicejet.com/CorporateQueries.aspx
- text: SpiceClub Loyalty Program Join our frequent flyer program and enjoy unmatched rewards, added comforts and exclusive privileges.
- link "Know More":
  - /url: https://spiceclub.spicejet.com/
- text: As a member you get access to Free Flight Vouchers Earn SC Points Complimentary Upgrades
- link "Join SpiceClub":
  - /url: https://spiceclub.spicejet.com/signup
- text: Cargo Small, Medium, Large & X-Large Get your cargo shipped from one place to another at a convenient and cost-effective manner. Book Now! Charter Your private space in the sky Now book a private charter for small groups or big. Book Now! About Us
- link "Spice Route Magazine":
  - /url: https://corporate.spicejet.com/SpiceRoute.aspx
- link "Corporate Overview":
  - /url: https://corporate.spicejet.com/CorporateOverview.aspx
- link "Fleet":
  - /url: https://corporate.spicejet.com/Fleet.aspx
- link "Careers":
  - /url: https://corporate.spicejet.com/careers.aspx
- link "Media Center":
  - /url: https://corporate.spicejet.com/PressReleaseNews.aspx
- link "Awards and Applaud":
  - /url: https://corporate.spicejet.com/AwardsandApplaud.aspx
- text: Contact Us
- link "Airports":
  - /url: https://corporate.spicejet.com/Airports.aspx
- link "International Offices":
  - /url: https://corporate.spicejet.com/InternationalOffices.aspx
- link "Corporate Head Office":
  - /url: https://corporate.spicejet.com/ContactUs.aspx
- link "Advertise with Us":
  - /url: https://corporate.spicejet.com/MediaSale.aspx
- link "RFP":
  - /url: https://corporate.spicejet.com/Content/pdf/GSA-RFP.pdf
- text: Travel Info
- link "Citizen's Charter":
  - /url: https://corporate.spicejet.com/Charter.aspx
- link "FAQ":
  - /url: https://corporate.spicejet.com/GeneralAirTravelFaq.aspx
- link "Fitness To Fly Guideline":
  - /url: https://corporate.spicejet.com/Content/pdf/FitnessFlyGuidelines.pdf
- link "Flight Schedules":
  - /url: https://corporate.spicejet.com/Schedules.aspx
- link "Terms of Carriage":
  - /url: https://corporate.spicejet.com/Tnc.aspx
- link "Fees and Charges":
  - /url: https://corporate.spicejet.com/FeesCharges.aspx
- link "Know Your Rights":
  - /url: https://www.civilaviation.gov.in/ministry-documents/passenger-charter-of-rights
- link "Book Any Test From Any Lab":
  - /url: http://flebo.in/
- link "Book Any Test From Any Lab in Delhi":
  - /url: https://flebo.in/health/delhi/
- text: Investors
- link "Corporate Governance":
  - /url: https://corporate.spicejet.com/InvestorsCorporateGovernance.aspx
- link "Financial Information":
  - /url: https://corporate.spicejet.com/InvestorsFinancialInformation.aspx
- link "Notices":
  - /url: https://corporate.spicejet.com/InvestorsNotices.aspx
- link "Corporate Announcements":
  - /url: https://corporate.spicejet.com/InvestorCorporateAnnouncements.aspx
- link "Shareholding Pattern":
  - /url: https://corporate.spicejet.com/InvestorsShareholdingPattern.aspx
- link "Investor Services":
  - /url: https://corporate.spicejet.com/InvestorServices.aspx
- link "Disclosure Regulation 46":
  - /url: https://corporate.spicejet.com/Regulation46.aspx
- text: "For Reservations or Customer Support: 📞 +91 (0)124 4983410 / +91 (0)124 7101600 📧"
- link "custrelations@spicejet.com":
  - /url: mailto:custrelations@spicejet.com
- text: Follow us
- link:
  - /url: https://www.facebook.com/FlySpiceJet/
- link:
  - /url: https://twitter.com/flyspicejet
- link:
  - /url: "https://www.linkedin.com/company/spicejet-limited/ "
- link:
  - /url: https://www.instagram.com/spicejetairlines/?hl=en
- link:
  - /url: https://www.youtube.com/channel/UCldnpuM5Q-y11XGNpfh3k-A
- text: Download our mobile app
- link:
  - /url: https://play.google.com/store/apps/details?id=com.vl.spicejet&hl=en
- link:
  - /url: https://itunes.apple.com/us/app/spicejet/id998319513?mt=8
- text: "Note: We log the IP addresses of the visitors on our website for security reasons. © Copyright Spicejet India. All Rights Reserved Registered Office: SpiceJet Ltd, 319, Udyog Vihar, Phase IV, Gurgaon, Haryana - 122016 Home •"
- link "Privacy Policy":
  - /url: https://corporate.spicejet.com/PrivacyPolicy.aspx
- text: •
- link "Disclaimer":
  - /url: "https://corporate.spicejet.com/Disclaimer.aspx "
- text: •
- link "GST Information":
  - /url: https://corporate.spicejet.com/GstInformation.aspx
- text: •
- link "Sitemap":
  - /url: https://corporate.spicejet.com/Sitemap.aspx
- text: Family and Friends discount is applicable for a minimum of 2 passengers (excluding infant) on a single booking. Terms and conditions apply. The special discounted fares are applicable to senior citizens, above the age of 60 years. To travel under this fare, the passenger must present a valid age proof at the SpiceJet check-in counter at the airport. Children between the ages of 5 and 12 travelling alone are considered unaccompanied minors. Charges per minor for each domestic sector are INR 4999, and charges vary for each international sector, starting at INR 9999. This can be bought until 2hrs/3hrs (domestic/international) prior to departure. After booking, please fill out the Unaccompanied Minor Form and carry four printed copies to the airport. Applicable for all students above the age of 12 years studying with a bonafide school/university. Limited inventory available. Valid photo ID and educational institute I-card need to be presented for verification at check-in. Bookings under this offer are refundable. Conditions apply. Applicable for all serving and retired Indian Armed Forces personnel, Paramilitary Forces personnel and their dependent families. Limited inventory available. Valid photo ID and relevant military card need to be presented for verification at check-in. Conditions apply. The fares are extended only to government officials for their official travel (those carrying a valid identity card). A valid identity card issued by the employer is a pre-requisite for travel under this offer and passengers will be required to produce the card at the SpiceJet check-in counter at the airport.
```

# Test source

```ts
  1  | import{test, expect} from '@playwright/test';
  2  | 
  3  | test("Verify keyboard hover and drag and drop", async({page})=>{
  4  | 
  5  |     await page.goto("https://www.spicejet.com/");
  6  |     await page.getByText("Add-ons",{exact:true}).hover();
  7  |     await page.getByText("FlyEarly",{exact:true}).click();
  8  | 
  9  |     let expectedUrl = "https://corporate.spicejet.com/FLYEarlyProductatAirports.aspx";
  10 | 
  11 |     //await expect(page).toHaveURL(expectedUrl);
> 12 |     await expect(page).toHaveTitle("SpiceJet");
     |                        ^ Error: expect(page).toHaveTitle(expected) failed
  13 |     
  14 | 
  15 | });
```